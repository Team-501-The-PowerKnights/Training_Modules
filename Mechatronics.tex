...
\subsection{Beater bars} %sidebar: why the heck don't we call these beater bars? idk.
...
\textit{Vectored intake wheels} are wheels with rollers at a 45 degree angle. These can be used with any beater bear or side wheel setup. They provide a centering action when properly implemented, as the rollers will cause a force vector 45 degrees to the axis of rotation, rather than perpindicular.
\subsection{Scoops}
A \textit{scoop} is pretty simple. It wedges under an object, picking it off the ground and into where you need it. They can be useful if the object to be picked up has a tapered bottom that is conducive to this and has much higher friction on the surface it is being picked up from versus the surface of the scoop.
\subsection{Vacuums}
\textit{Vacuum pumps} can be used to acquire objects that can form a sufficiently good seal. They are a little difficult to set up properly, and are also notably inefficient and loud, but can be used properly and effectively.
...
\textit{Conveyors} are the most common type of indexer. Objects are fed single file from one place to another using continuous belts. Variants of this exist, such as dual-conveyors (where objects are passed between two conveyors, so they do not need to rotate, but are still well-constrained).
\subsection{Rotary Hoppers}
\textit{Rotary hoppers} are like gravity hoppers, but use some sort of rotating piece at the bottom to both agitate objects in the hopper and forcefully feed them to the next place. These allow for high capacity and control.
\subsection{Agitated Hoppers}
\textit{Agitated hoppers} are slightly angled hoppers which also feature moving sidewalls or rollers that help agitate objects and may even influence them to move faster than gravity would normally allow for. These can be simple to design and integrate while allowing for greater reliability than a gravity hopper. 
...
An \textit{elevator} is a mechanism that moves back and forth in a straight line. \textit{Cascade} and \textit{Continous} elevators are able to reach further than their initial size without additional degrees of freedom by using special rigging of string to pull multiple stages that are nested in each other.

Cascade elevators have multiple strings to consider. They also have mechanical disadvantage by a factor of the number of stages in the lift. Continuous elevators do not have this mechanical disadvantage and only have one string to worry about. This single string can also be used to pull the elevator down in addition to upwards, making them less reliant on gravity.
...
\subsection{General 4 Bars}
\textit{4-bar linkages} are very versatile mechanisms that can achieve various behaviors.
\subsection{Parallel 4 Bar}
\textit{Parallel 4 bar linkages} are often what people refer to when they say ``4-bar mechanism". The links on these form a parallelogram. These are simple to make but do not lift straight up and down- they will move outwards in an arc as they rise. This may be a desirable trait.
...
%\section{Latches}
%\textit{Latches} grab onto things and don't let go.
%\subsection{Latches}
...
\textit{Opposed wheel} shooters use mutiple sets of wheels to contact the ball. This allows for control over backspin, and may be easier to package.
...
\begin{asparaenum}[a)]
\item A \textit{choo-choo} uses a rotating plate with a pin, a link, and another link or string. As the plate is rotated, the pin makes contact with the link. Eventually the second link/string gets pulled back and goes over-center from this pin, so that the pin no longer supports the link. At this point the spring pulls the mechanism forwards as it fires.

\item In a \textit{winch and release} a winch pulls back the sled/cradle. When firing is desired, a release mechanism (like those discussed in section \ref{section:disengagement}) allows the sled/cradle to fly forth.
...
\item \textit{Pneumatic cylinders} are very simple ways of creating sudden bursts of energy.
\end{asparaenum}
